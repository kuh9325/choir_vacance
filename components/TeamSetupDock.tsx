...
export function TeamSetupDock() {
  ...
  const saveCharades = async (mutate: (draft: CharadesState) => void) => {
    ...
    channel.close();
    setSaving(true);
    try { await fetch('/api/charades', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(next) }); }
    finally { setSaving(false); }
  };

  const changeTeamCount = (count: number) => {
    void saveScore(`참가팀 수를 ${count}팀으로 변경`, (draft) => {
      if (count > draft.teams.length) draft.teams.push(...Array.from({ length: count - draft.teams.length }, (_, index) => makeTeam(draft.teams.length + index)));
      else draft.teams = draft.teams.slice(0, count);
  ...
  return <>
    <button className={styles.trigger} onClick={() => setOpen(true)} aria-label="팀 구성 설정 열기"><span>👥</span><b>팀 구성</b><small>{score.teams.length}팀 · {score.teams.reduce((sum, team) => sum + (charades.memberCounts[team.id] ?? 4), 0)}명</small></button>

    {open && <div className={styles.backdrop} role="dialog" aria-modal="true" aria-label="팀 구성 설정"><section className={styles.panel}>
      <header><div><p>TEAM SETUP</p><h2>팀 구성 설정</h2><span>점수판·몸으로 말해요·텔레스트레이션에 함께 반영됩니다.</span></div><button onClick={() => setOpen(false)} aria-label="닫기">×</button></header>

      <div className={styles.countRow}><div><b>참가 팀 수</b><span>3~8팀</span></div><div className={styles.stepper}><button disabled={score.teams.length <= 3} onClick={() => changeTeamCount(score.teams.length - 1)}>−</button><strong>{score.teams.length}팀</strong><button disabled={score.teams.length >= 8} onClick={() => changeTeamCount(score.teams.length + 1)}>＋</button></div></div>

      {score.teams.length > 5 && <div className={styles.warning}>텔레스트레이션도 최대 8팀까지 지원합니다. 팀 수가 늘어나면 제시어 카드와 판정 화면이 자동으로 늘고 단계 시간이 자동 조정됩니다.</div>}

      <div className={styles.teamList}>{score.teams.map((team, index) => {
        ...
        return <article key={team.id} style={{ '--team': team.color } as React.CSSProperties}>
          ...
          <div className={styles.members}><span>인원</span><div><button disabled={members <= 1} onClick={() => void saveCharades((draft) => { draft.memberCounts[team.id] = Math.max(1, members - 1); })}>−</button><strong>{members}명</strong><button disabled={members >= 8} onClick={() => void saveCharades((draft) => { draft.memberCounts[team.id] = Math.min(8, members + 1); })}>＋</button></div></div>
        </article>;
      })}</div>

      <footer><span>{saving ? '저장 중…' : '✓ 변경 즉시 자동 저장'}</span><button onClick={() => setOpen(false)}>완료</button></footer>
    </section></div>}
  </>;
}
