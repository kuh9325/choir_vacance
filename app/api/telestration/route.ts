...
import { DEFAULT_TELESTRATION_STATE, normalizeTelestrationState } from '@/lib/telestration';
...
export async function GET() {
  ...
  if (!row) return Response.json(DEFAULT_TELESTRATION_STATE, { headers: { 'Cache-Control': 'no-store' } });
  try {
    return Response.json({ ...normalizeTelestrationState(JSON.parse(row.payload)), revision: row.revision, updatedAt: row.updated_at }, { headers: { 'Cache-Control': 'no-store' } });
  } catch {
    return Response.json(DEFAULT_TELESTRATION_STATE, { headers: { 'Cache-Control': 'no-store' } });
  }
}
...
export async function PUT(request: Request) {
  ...
  let state;
  try {
    state = normalizeTelestrationState(JSON.parse(rawText));
  } catch {
